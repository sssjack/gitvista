...
function DiffViewer({ diff, split, wordWrap = false, fontSize = 12, path }: { diff: DiffResult | null; split: boolean; wordWrap?: boolean; fontSize?: number; path?: string }) {
  ...
  const empty = !diff ? ['选择文件以查看差异', <FileDiff size={30} />] : !diff.text.trim() ? [diff.binary ? '二进制内容发生变化' : '没有文本差异', diff.binary ? <Files size={30} /> : <CheckCheck size={30} />] : null;
  if (empty) return <div className="empty-state"><div className="empty-icon">{empty[1]}</div><strong>{empty[0]}</strong></div>;
  const warning = diff?.truncated && <div className="inline-warning">差异内容较大，已截断显示。请使用外部编辑器检查完整文件。</div>;
  ...
  const side = (name: 'left' | 'right') => <section className="diff-side" aria-label={name === 'left' ? '原始版本' : '修改后'}>
    <div className="diff-side-heading">{name === 'left' ? '原始版本' : '修改后'}</div>
    <div className="diff-side-scroll" ref={name === 'left' ? left : right} tabIndex={0} onScroll={event => {
      ...
      if (pair.kind === 'gap') return <div className="diff-gap" role="separator" aria-label="中间未变化的代码已省略" title="中间未变化的代码已省略" data-diff-row={index} key={index}><span>···</span></div>;
      const row = pair[name];
  ...
  return <div className={`diff-viewer ${wordWrap ? 'wrap-lines' : ''}`} ref={root}>
    {files.length > 1 && <div className="diff-file-tabs" role="tablist" aria-label="差异文件">{files.map(item => <button role="tab" aria-selected={item.key === file?.key} className={item.key === file?.key ? 'active' : ''} key={item.key} title={item.path} onClick={() => setSelectedFile(item.key)}><FileDiff size={12} />{item.path}</button>)}</div>}
    {!!pairs?.length && <div className="diff-change-legend" aria-label="差异颜色说明"><span className="add"><i />新增 {file.counts.add}</span><span className="modify"><i />修改 {file.counts.modify}</span><span className="remove"><i />删除 {file.counts.remove}</span>{file.summary.length > 0 && <span className="diff-file-summary" title={file.summary.join('；')}>{file.summary.join('；')}</span>}</div>}
    {!pairs?.length ? <div className="empty-state diff-information"><div className="empty-icon"><Files size={30} /></div><strong>{file?.binary || files.length === 1 && diff?.binary ? '二进制内容发生变化' : '仅文件信息发生变化'}</strong>{(file?.summary || ['未包含可显示的文本差异。']).map((text, index) => <p key={index}>{text}</p>)}</div> : split ? <div className="diff-split-layout" style={{ gridTemplateColumns: `minmax(0,${ratio}fr) 6px minmax(0,${1 - ratio}fr)` }}>
      {side('left')}<ResizeHandle direction="vertical" label="调整左右差异宽度" onResize={delta => setRatio(current => clamp(current + delta / Math.max(1, root.current?.clientWidth || 1), .2, .8))} onReset={() => setRatio(.5)} />{side('right')}
    </div> : <div className="diff-scroll unified" tabIndex={0}><div className="diff-code">{file.unified.map((row, index) => row.kind === 'gap' ? <div className="diff-gap" role="separator" aria-label="中间未变化的代码已省略" title="中间未变化的代码已省略" key={index}><span>···</span></div> : <div className={`diff-line ${row.kind}`} data-change={row.kind} key={index}><span className="line-number">{row.line.old}</span><span className="line-number">{row.line.next}</span><code>{code(row.line, row.line.kind === 'remove' ? 'left' : 'right')}</code></div>)}</div></div>}
    {warning}
    {syntax.limited && <div className="syntax-limit-note">较长差异保留原文，部分语法颜色已简化。</div>}
  </div>;
}
