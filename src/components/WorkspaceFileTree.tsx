...
type Props = { root: string; revision: number; selected?: string; onSelect: (path: string) => void; statuses: Map<string, string>; onContextMenu?: (event: React.MouseEvent<HTMLButtonElement>, path: string) => void };
function Directory({ root, revision, selected, onSelect, statuses, onContextMenu, path = '', name = '', depth = -1 }: Props & { path?: string; name?: string; depth?: number }) {
  const { t } = useI18n();
  ...
  return <div>
    ...
    {expanded && <>
      ...
      {listing?.entries.map(entry => entry.type === 'directory'
        ? <Directory key={entry.path} root={root} revision={revision} selected={selected} onSelect={onSelect} statuses={statuses} onContextMenu={onContextMenu} path={entry.path} name={entry.name} depth={depth + 1} />
        : <button key={entry.path} className={`tree-file ${selected === entry.path ? 'active' : ''}`} data-workspace-file={entry.path} title={entry.type === 'link' ? `${entry.path} · ${t('符号链接不可预览')}` : entry.path} disabled={entry.type === 'link'} style={{ paddingLeft: 22 + (depth + 1) * 13, paddingRight: 9 }} onClick={() => onSelect(entry.path)} onContextMenu={onContextMenu && entry.type !== 'link' ? event => onContextMenu(event, entry.path) : undefined}>{entry.type === 'link' ? <Link size={14} /> : <FileIcon path={entry.path} />}<span className="tree-file-name">{entry.name}</span><i>{statuses.get(entry.path)}</i></button>)}
