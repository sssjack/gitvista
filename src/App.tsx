...
import { useFileHistory } from './components/FileHistory';
...
import PushCommitFiles from './components/PushCommitFiles';
import PushFileDiff, { PushDiffContents, usePushFileDiff } from './components/PushFileDiff';
...
function Workbench({ onLanguageChange }: { onLanguageChange: (language: AppLanguage) => void }) {
  ...
  const [repo, setRepo] = useState(''); const fileHistory = useFileHistory(repo, settings); const [snapshot, setSnapshot] = useState<GitSnapshot | null>(null);
  ...
  useEffect(() => { const listener = (e: KeyboardEvent) => { if (document.querySelector('.mini-bar, .mini-edge')) return; if ((e.ctrlKey || e.metaKey) && e.altKey && e.key.toLowerCase() === 's') { e.preventDefault(); if (!busy && !tool && !cloneOpen && !credentialsOpen && !result && !settingsOpen && !pushOpen) setSettingsOpen(true); } if ((e.ctrlKey || e.metaKey) && e.key.toLowerCase() === 'o') { e.preventDefault(); if (!busy && !tool && !cloneOpen && !credentialsOpen && !result && !settingsOpen && !pushOpen) void openRepo(); } if ((e.ctrlKey || e.metaKey) && e.key.toLowerCase() === 'r') { e.preventDefault(); if (!settingsOpen && !busy && !tool && !cloneOpen && !credentialsOpen && !result && !pushOpen) void refresh(); } if ((e.ctrlKey || e.metaKey) && e.key === 'Enter' && (e.target as HTMLElement).classList.contains('commit-message') && !tool && !cloneOpen && !credentialsOpen && !result && !settingsOpen && !pushOpen) { e.preventDefault(); void commit(); } if (e.key === 'Escape' && !busy) { setTool(null); setMore(false); setResult(null); setCloneOpen(false); } }; window.addEventListener('keydown', listener); return () => window.removeEventListener('keydown', listener); }, [openRepo, refresh, commit, tool, cloneOpen, credentialsOpen, result, busy, settingsOpen, pushOpen]);
  ...
  useEffect(() => { const listener = () => { if (repoRef.current && !mutationRef.current && Date.now() - lastRefreshAt.current > 1500 && !busy && !tool && !cloneOpen && !credentialsOpen && !result && !settingsOpen && !pushOpen) void refresh().then(() => setSelection(current => current ? { ...current } : null)); }; window.addEventListener('focus', listener); return () => window.removeEventListener('focus', listener); }, [busy, tool, cloneOpen, result, settingsOpen, refresh]);
  ...
  const fileRow = (file: GitFile, isStaged: boolean) => {
    ...
    return <div className={`change-row ${selection?.kind === 'file' && selection.path === file.path && selection.staged === isStaged ? 'selected' : ''} ${ticked ? 'ticked' : ''}`} key={`${file.path}-${isStaged}`} aria-busy={pending} onContextMenu={event => fileHistory.context(event, { repo, path: file.path, oldPath: file.index === 'R' || file.worktree === 'R' ? file.oldPath : undefined })}>
    ...
    {workspaceEntry ? <WorkspacePanel key={workspaceEntry.path} root={workspaceEntry.path} command={workspaceCommand} layout={{ sidebar, dockTab, onDock: toggleDock, historyOpen, onHistory: () => setHistoryOpen(value => !value), settings }} onOpen={openWorkspaceRepository} onBusy={setBusy} /> : !snapshot ? <main className="welcome"><div className="welcome-content"><h1>{t('打开一个仓库')}</h1><p>{t('从本地目录、远程地址或一个全新的空仓库开始。')}</p><div className="welcome-actions"><button className="primary-button" onClick={() => void openRepo()}><FolderOpen size={16} />{t('打开本地仓库')}</button><button className="secondary-button" onClick={() => setCloneOpen(true)}><ArrowDownToLine size={15} />{t('克隆仓库')}</button></div><div className="welcome-hint"><button className="text-button" onClick={async () => { try { const r = await api.initRepository(); if (r) await selectRepo(r); } catch (e) { setError(ErrorText(e)); } }}><Plus size={14} />{t('初始化一个新仓库')}</button><span className="hint-separator">·</span><kbd>Ctrl</kbd><span>+</span><kbd>O</kbd><span>{t('快速打开')}</span></div>{settings.repos.length > 0 && <div className="recent-repos"><div className="subtle-heading">{t('最近使用')}</div>{settings.repos.slice(0, 5).map(r => <button key={r.path} onClick={() => void openRepo(r.path)}><FolderGit2 size={16} /><span>{r.name}<small>{r.path}</small></span><ChevronRight size={14} /></button>)}</div>}</div>{loading && <div className="loading-overlay"><Loader2 className="spin" size={22} /><span>{t('正在读取仓库…')}</span></div>}</main> : <div ref={workspaceRef} className={`workspace ${sidebar ? '' : 'sidebar-hidden'}`} style={{ '--side-width': `${renderedSide}px`, '--changes-width': `${renderedChanges}px`, '--history-height': `${historyHeight}px` } as React.CSSProperties}>
      ...
      <main className="main-workspace" ref={mainRef}>
        ...
        {historyOpen ? <div className="history-dock" style={{ height: historyHeight }}>
          <LogPanel key={repo} repo={repo} snapshot={snapshot} selected={selection?.kind === 'commit' ? selection.hash : undefined} busy={!!busy} blocked={!!tool || cloneOpen || credentialsOpen || !!result || settingsOpen || fileHistory.isOpen} refreshing={loading} command={logCommand} inCurrentBranch={detail?.commit.hash === (selection?.kind === 'commit' ? selection.hash : undefined) ? detail?.inCurrentBranch : undefined} onSelect={hash => setSelection({ kind: 'commit', hash })} onRefresh={() => void refresh()} onTool={openTool} />
    ...
    {pushOpen && <div className="modal-backdrop" onMouseDown={e => { if (e.target === e.currentTarget && !busy) { setPushOpen(false); pushFile.clear(); } }}><div className="push-modal" inert={pushDiff?.open || undefined} role="dialog" aria-modal="true" aria-label={t('推送预览')}>
      ...
      <div className="push-body">
        ...
        <div className="push-preview-pane">
          {pushDiff ? <><div className="diff-filebar"><FileIcon path={pushDiff.path} size={14} /><span className="diff-file-path" title={pushDiff.path}>{pushDiff.path}</span><span className="diff-stage-label">{pushDiff.source === 'commit' ? pushDiff.ref?.slice(0, 8) : t('已暂存版本')}</span><IconButton title={t('弹窗查看差异')} onClick={pushFile.reopen}><Maximize2 size={13} /></IconButton></div><PushDiffContents state={pushDiff} preferences={settings} split={split} /></> : <Empty title={t('待推送文件差异')} detail={t('展开左侧提交，点击文件查看差异；关闭弹窗后可在此继续查看。')} icon={<FileDiff size={28} />} />}
