...
export default function App() {
  ...
  useEffect(() => {
    let alive = true;
    void window.gitvista.settings().then(value => { if (alive) setLanguage(value.language === 'zh' ? 'zh' : 'en'); }).catch(() => { /* 首次运行或读取失败时保持英文。 */ });
    return () => { alive = false; };
  }, []);
  return <I18nProvider language={language}><Workbench onLanguageChange={setLanguage} /></I18nProvider>;
}
...
function Workbench({ onLanguageChange }: { onLanguageChange: (language: AppLanguage) => void }) {
  ...
  const selectionKey = (path: string, staged: boolean) => `${staged ? 'i' : 'w'} ${path}`;
  ...
  const setSelectedAll = useCallback((paths: string[], staged: boolean, on: boolean) => setSelected(current => {
    const base = current.repo === repoRef.current ? current : { repo: repoRef.current, staged: new Set<string>(), unstaged: new Set<string>() };
    const next = new Set(on ? paths.map(path => selectionKey(path, staged)) : []);
    return { ...base, [staged ? 'staged' : 'unstaged']: next };
  }), []);
  ...
  const selectRepo = useCallback(async (entry: RepoEntry) => { setError(''); setNotice(''); setResult(null); setTool(null); setCloneOpen(false); setSettingsOpen(false); ++amendRequestId.current; repoRef.current = entry.path; setRepo(entry.path); setAmend(false); setMessage(''); setSnapshot(null); setSelection(null); setDiff(null); setDetail(null); setLogCommand(undefined); setBranchFilter(''); setAllFiles(false); setTreeEntries([]); setCode(null); setSelected({ repo: entry.path, staged: new Set(), unstaged: new Set() }); await Promise.all([reloadSettings(), refresh(entry.path, true)]); }, [refresh, reloadSettings]);
  ...
  useEffect(() => {
    setSelected(current => {
      ...
      const liveStaged = new Set(files.map(file => selectionKey(file.path, true)));
      const liveUnstaged = new Set(files.map(file => selectionKey(file.path, false)));
  ...
  useEffect(() => { const listener = (event: KeyboardEvent) => { if ((event.ctrlKey || event.metaKey) && event.key.toLowerCase() === 'd' && !event.altKey && !busy && !tool && !cloneOpen && !result && !settingsOpen && activePath && !diffLoading) { event.preventDefault(); void openFileDiff(); } }; window.addEventListener('keydown', listener); return () => window.removeEventListener('keydown', listener); }, [busy, tool, cloneOpen, result, settingsOpen, activePath, openFileDiff, diffLoading]);
  useEffect(() => { const listener = () => { if (repoRef.current && !mutationRef.current && Date.now() - lastRefreshAt.current > 1500 && !busy && !tool && !cloneOpen && !result && !settingsOpen) void refresh().then(() => setSelection(current => current ? { ...current } : null)); }; window.addEventListener('focus', listener); return () => window.removeEventListener('focus', listener); }, [busy, tool, cloneOpen, result, settingsOpen, refresh]);
  ...
  const fileRow = (file: GitFile, isStaged: boolean) => {
    ...
    return <div className={`change-row ${selection?.kind === 'file' && selection.path === file.path && selection.staged === isStaged ? 'selected' : ''} ${ticked ? 'ticked' : ''}`} key={`${file.path}-${isStaged}`} aria-busy={pending}>
      ...
      <button className="change-file" title={file.path} onClick={() => setSelection({ kind: 'file', path: file.path, staged: isStaged })}>{pending ? <Loader2 size={14} className="spin" aria-label={t('正在更新暂存状态')} /> : <FileIcon path={file.path} size={14} />}<span className={fileStatusClass(workingFileStatus(file, isStaged))}>{basename(file.path)}<small>{file.path.includes('/') ? file.path.slice(0, file.path.lastIndexOf('/')) : t('根目录')}</small></span></button>
  ...
  return <div className={`app-shell ${settings.wordWrap ? 'code-wrap' : ''}`} style={{ '--code-font-size': `${settings.codeFontSize}px`, '--code-line-height': `${Math.round(settings.codeFontSize * 1.8)}px` } as React.CSSProperties}>
    <header className="titlebar"><div className="brand"><span className="brand-mark"><GitFork size={14} strokeWidth={2.5} /></span><span>GitVista</span></div><div className="titlebar-center">{snapshot ? <><FolderGit2 size={13} />{snapshot.name}<span className="title-separator">/</span><span>{t('版本控制工作台')}</span></> : '让每一次变更，都清晰可见'}</div><div className="window-controls"><button title={t('最小化')} onClick={() => api.windowControl('minimize')}><Minus size={14} /></button><button title={t('最大化 / 还原')} onClick={() => api.windowControl('maximize')}><Maximize2 size={12} /></button><button className="window-close" title={t('关闭窗口')} onClick={() => api.windowControl('close')}><X size={15} /></button></div></header>
    <div className="toolbar"><div className="repo-control"><IconButton title={sidebar ? t('收起侧栏') : t('展开侧栏')} onClick={() => setSidebar(!sidebar)}>{sidebar ? <PanelLeftClose size={17} /> : <PanelLeftOpen size={17} />}</IconButton><FolderGit2 size={17} /><SelectMenu label="选择仓库" className="repo-picker" placeholder="选择仓库" searchable searchPlaceholder="搜索仓库名称或路径…" value={repo} disabled={!!busy} onChange={value => { if (value === '__open') void openRepo(); else void openRepo(value); }} options={[...settings.repos.map(r => ({ value: r.path, label: r.name, description: r.path, icon: <FolderGit2 size={16} />, group: '最近仓库' })), { value: '__open', label: '打开其他仓库…', icon: <Plus size={16} />, group: '仓库操作' }]} /><IconButton title={t('打开仓库 · Ctrl+O')} onClick={() => void openRepo()}><Plus size={15} /></IconButton></div>{snapshot && <><div className="toolbar-divider" /><div className="branch-control"><GitBranch size={15} /><SelectMenu label="切换当前分支" className="branch-picker" searchable searchPlaceholder="搜索本地分支…" value={snapshot.branch} disabled={!!busy} onChange={value => void runAction({ type: 'switch', ref: value }, '切换分支')} options={[...(!snapshot.branches.some(b => !b.remote && b.name === snapshot.branch) ? [{ value: snapshot.branch, label: snapshot.branch || '分离 HEAD', disabled: true }] : []), ...snapshot.branches.filter(b => !b.remote).map(b => ({ value: b.name, label: b.name, icon: <GitBranch size={15} />, group: '本地分支' }))]} /></div><span className="sync-count" title="领先 / 落后远端">↑ {snapshot.ahead} <span>↓ {snapshot.behind}</span></span></>}<div className="toolbar-spacer" />{snapshot && <div className="sync-actions"><button disabled={!!busy} onClick={() => void runAction({ type: 'fetch' }, t('获取远端'))} title={t('获取最新远端引用')}><RefreshCw size={14} />{t('获取')}</button><button disabled={!!busy} onClick={() => void runAction({ type: 'pull', mode: settings.pullStrategy }, t('拉取更新'))} title={`${t('拉取更新')} · ${settings.pullStrategy}`} ><ArrowDown size={15} />{t('拉取')}</button><button className="push-button" disabled={!!busy} onClick={() => void openPush()} title={t('推送当前分支')}><ArrowUp size={15} />{t('推送')}{snapshot.ahead > 0 && <span>{snapshot.ahead}</span>}</button></div>}<div className="toolbar-divider" /><div className="theme-control">{settings.theme === 'light' ? <Sun size={15} /> : <Moon size={15} />}<SelectMenu label={t('选择主题')} className="theme-picker" align="end" value={settings.theme} onChange={value => void changeTheme(value as AppTheme)} options={THEMES.map(theme => ({ value: theme.value, label: t(theme.name), description: t(theme.detail), icon: <span className="theme-option-swatch" style={{ background: theme.colors[0], borderColor: theme.colors[2] }}>{theme.colors.slice(1).map((color, index) => <i key={index} style={{ background: color }} />)}</span> }))} /></div><IconButton title="Git 工具箱" onClick={() => openTool('branchCreate')} disabled={!snapshot}><Command size={17} /></IconButton><IconButton title={t('恢复默认面板布局')} onClick={resetLayout} disabled={!snapshot}><RotateCcw size={16} /></IconButton><IconButton title={t('应用设置')} onClick={() => setSettingsOpen(true)} disabled={!!busy}><Settings2 size={17} /></IconButton></div>
    ...
    {!snapshot ? <main className="welcome"><div className="welcome-content"><h1>{t('打开一个仓库')}</h1><p>{t('从本地目录、远程地址或一个全新的空仓库开始。')}</p><div className="welcome-actions"><button className="primary-button" onClick={() => void openRepo()}><FolderOpen size={16} />{t('打开本地仓库')}</button><button className="secondary-button" onClick={() => setCloneOpen(true)}><ArrowDownToLine size={15} />{t('克隆仓库')}</button></div><div className="welcome-hint"><button className="text-button" onClick={async () => { try { const r = await api.initRepository(); if (r) await selectRepo(r); } catch (e) { setError(ErrorText(e)); } }}><Plus size={14} />{t('初始化一个新仓库')}</button><span className="hint-separator">·</span><kbd>Ctrl</kbd><span>+</span><kbd>O</kbd><span>{t('快速打开')}</span></div>{settings.repos.length > 0 && <div className="recent-repos"><div className="subtle-heading">{t('最近使用')}</div>{settings.repos.slice(0, 5).map(r => <button key={r.path} onClick={() => void openRepo(r.path)}><FolderGit2 size={16} /><span>{r.name}<small>{r.path}</small></span><ChevronRight size={14} /></button>)}</div>}</div>{loading && <div className="loading-overlay"><Loader2 className="spin" size={22} /><span>{t('正在读取仓库…')}</span></div>}</main> : <div ref={workspaceRef} className={`workspace ${sidebar ? '' : 'sidebar-hidden'}`} style={{ '--side-width': `${renderedSide}px`, '--changes-width': `${renderedChanges}px`, '--history-height': `${historyHeight}px` } as React.CSSProperties}>
      ...
      {sidebar && dockTab === 'changes' && <aside className="changes-panel" aria-label={t('本地变更') + ' ' + t('工具窗口')}><div className="panel-heading"><div className="panel-tab"><Files size={15} />{t('本地变更')}<span>{files.length}</span></div><IconButton title={t('刷新本地变更')} disabled={!!busy} onClick={() => void refreshWorking(repo).then(() => setNotice(t('刷新工作区状态，显示最新修改')))}><RefreshCw size={14} className={loading ? 'spin' : ''} /></IconButton><IconButton title={t('暂存到 Stash')} disabled={!!busy || !files.length} onClick={() => openTool('stashSave')}><Layers3 size={15} /></IconButton></div><div className="changes-branch"><span className="live-dot" /><GitBranch size={13} /><strong>{snapshot.branch}</strong><span>{snapshot.upstream ? `${t('跟踪')} ${snapshot.upstream}` : t('本地分支状态')}</span></div>{conflicts.length > 0 && <div className="conflict-notice"><span>!</span>{conflicts.length} {t('个文件需要解决冲突')}</div>}<div className="change-groups" ref={groupsRef}>
        <section className="change-group unstaged-group" style={{ flexBasis: `${stageRatio * 100}%` }} aria-label={t('未暂存')}><div className="change-group-title"><ChevronDown size={12} /><strong>{t('未暂存')}</strong><span>{unstaged.length}</span><button disabled={!!busy || !unstaged.some(f => !f.conflict)} onClick={() => setSelectedAll(unstaged.filter(f => !f.conflict).map(f => f.path), false, true)}>{t('全选')}</button></div><div className="change-group-list">{unstaged.map(f => fileRow(f, false))}{!unstaged.length && <div className="change-empty">{t('工作区没有未暂存更改')}</div>}</div></section>
      ...
      <main className="main-workspace" ref={mainRef}>
        <section className="diff-panel"><div className="panel-heading"><div className="panel-tab"><FileDiff size={15} />{selection?.kind === 'file' ? t('工作区代码') : t('历史代码')}{detail && <span>{detail.files.length} 个文件</span>}</div><div className="panel-heading-actions"><div className="segmented view-toggle"><button className={!codeMode ? 'active' : ''} onClick={() => setCodeMode(false)}>差异</button><button className={codeMode ? 'active' : ''} onClick={() => setCodeMode(true)}>完整代码</button></div><div className="segmented"><button className={!split ? 'active' : ''} onClick={() => setSplit(false)} title="统一差异">统一</button><button className={split ? 'active' : ''} onClick={() => setSplit(true)} title="并排差异">并排</button></div><IconButton title={settings.wordWrap ? t('关闭代码自动换行') : t('开启代码自动换行')} className={settings.wordWrap ? "active" : ""} onClick={() => { const next = { ...settings, wordWrap: !settings.wordWrap }; setSettings(next); void api.updatePreferences(next).catch(e => setError(ErrorText(e))); }}><WrapText size={15} /></IconButton><IconButton title="文件历史" disabled={!activePath} onClick={() => openTool('fileHistory', { path: activePath || '', ref: contentRef || '' })}><History size={14} /></IconButton><IconButton title="逐行追溯" disabled={!activePath} onClick={() => openTool('blame', { path: activePath || '' })}><Code2 size={14} /></IconButton></div></div><div className="diff-body"><><div className={`commit-files ${showPreview ? '' : 'without-preview'}`} style={{ width: showPreview ? renderedFiles : undefined }}><div className="file-directory-heading"><FolderOpen size={14} /><strong>文件目录</strong><span>{selection?.kind === 'commit' ? selectedCommit?.short : snapshot.branch}</span></div>{detail && <><div className="tree-mode"><button className={!allFiles ? 'active' : ''} onClick={() => { setAllFiles(false); controlTree(false); }}>改动文件</button><button className={allFiles ? 'active' : ''} onClick={() => { setAllFiles(true); setCodeMode(true); controlTree(true); }}>全部文件</button></div><div className="commit-file-toolbar" aria-label="历史文件工具栏"><IconButton title="显示差异 · Ctrl+D" disabled={!activePath || diffLoading || !!busy} onClick={() => void openFileDiff()}><ArrowLeftRight size={14} /></IconButton><IconButton title="反向撤销所选历史文件改动" disabled={!activePath || !!busy || allFiles} onClick={() => openTool('revertFile', { ref: detail.commit.hash, path: activePath || '' })}><Undo2 size={14} /></IconButton><IconButton title="截至此提交的文件历史" disabled={!activePath || !!busy} onClick={() => openTool('fileHistory', { path: activePath || '', ref: detail.commit.hash })}><History size={14} /></IconButton><span className="file-toolbar-divider" /><IconButton title="展开全部目录" onClick={() => controlTree(false)}><ChevronsUpDown size={14} /></IconButton><IconButton title="折叠全部目录" onClick={() => controlTree(true)}><ChevronsDownUp size={14} /></IconButton><IconButton title={showCommitDetails ? '隐藏提交详情' : '显示提交详情'} className={showCommitDetails ? 'active' : ''} onClick={() => setShowCommitDetails(value => !value)}><Eye size={14} /></IconButton><IconButton title={showPreview ? '隐藏差异预览' : '显示差异预览'} onClick={() => setShowPreview(value => !value)}>{showPreview ? <PanelRightClose size={14} /> : <PanelRightOpen size={14} />}</IconButton></div></>}{!detail && <div className="commit-file-toolbar" aria-label="工作区目录工具栏"><IconButton title="展开全部目录" onClick={() => controlTree(false)}><ChevronsUpDown size={14} /></IconButton><IconButton title="折叠全部目录" onClick={() => controlTree(true)}><ChevronsDownUp size={14} /></IconButton></div>}<FileTree key={treeScope} files={directoryFiles} selected={activePath} control={effectiveTreeControl} onSelect={selectDirectoryFile} /></div>{showPreview && <ResizeHandle direction="vertical" label="调整文件目录与代码宽度" onResize={delta => setFileWidth(current => clamp(clamp(current, 130, (mainRef.current?.clientWidth || 600) - 360) + delta, 130, Math.max(130, (mainRef.current?.clientWidth || 600) - 240)))} onReset={() => setFileWidth(245)} />}</>{(showPreview || !detail) && <div className="diff-content">{activePath && <div className="diff-filebar"><FileIcon path={activePath} size={14} /><span className="diff-file-path" title={activePath}>{activePath}</span>{deletedFile && codeMode && <span className="diff-stage-label">删除前版本</span>}{selection?.kind === 'file' && <span className="diff-stage-label">{selection.staged ? '已暂存' : '未暂存'}</span>}<IconButton title="在文件管理器中显示" onClick={() => void api.revealPath(repo, activePath).catch(e => setError(ErrorText(e)))}><ArrowUpRight size={12} /></IconButton></div>}{(codeMode ? codeLoading : diffLoading) ? <div className="inline-loading"><Loader2 size={18} className="spin" />{t(codeMode ? '读取代码…' : '读取差异…')}</div> : noticeFile?.repo === repo && noticeFile.path === activePath ? <div className="soft-notice" role="status"><div className="soft-notice-icon"><FileWarning size={26} /></div><strong>{t('无法预览该文件')}</strong><p>{noticeFile.message}</p><div className="soft-notice-actions"><button type="button" onClick={() => void api.revealPath(repo, activePath).catch(() => setNotice(t('在文件管理器中显示')))}><ArrowUpRight size={13} />{t('在文件管理器中显示')}</button></div></div> : codeMode ? <CodeViewer text={code} path={activePath} /> : <DiffViewer diff={diff} path={activePath} split={split} wordWrap={settings.wordWrap} fontSize={settings.codeFontSize} />}</div>}</div></section>
        ...
        {historyOpen ? <div className="history-dock" style={{ height: historyHeight }}>
          <LogPanel key={repo} repo={repo} snapshot={snapshot} selected={selection?.kind === 'commit' ? selection.hash : undefined} busy={!!busy} blocked={!!tool || cloneOpen || !!result || settingsOpen} refreshing={loading} command={logCommand} inCurrentBranch={detail?.commit.hash === (selection?.kind === 'commit' ? selection.hash : undefined) ? detail?.inCurrentBranch : undefined} onSelect={hash => setSelection({ kind: 'commit', hash })} onRefresh={() => void refresh()} onTool={openTool} />
          {showCommitDetails && <><ResizeHandle direction="vertical" label="调整历史与提交详情宽度" onResize={delta => setDetailsWidth(current => clamp(clamp(current, 240, (workspaceSize.width - 49 - renderedSide) * .43) - delta, 240, Math.min(560, (workspaceSize.width - 49 - renderedSide) * .43)))} onReset={() => setDetailsWidth(340)} /><aside className="history-details" style={{ width: renderedDetails }} aria-label="提交信息"><div className="panel-heading"><div className="panel-tab"><GitCommitHorizontal size={15} />提交信息</div><IconButton title="收起提交信息" onClick={() => setShowCommitDetails(false)}><X size={14} /></IconButton></div>{detail ? <CommitDetails detail={detail} root={detail.root || repo} onBranch={branch => requestLog('branch', branch)} onCopy={hash => void navigator.clipboard.writeText(hash).then(() => setNotice('提交哈希已复制')).catch(e => setError(ErrorText(e)))} /> : <Empty title="选择一条提交记录" detail="在左侧历史中查看作者、说明和所属分支。" />}</aside></>}
