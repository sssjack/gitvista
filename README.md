...
内置 Git · 多仓库批量同步 · 默认英文 / 支持中文 · 可调工作台 · 托盘与迷你横条
...
- **父文件夹就是多仓库工作区。** 自动发现子目录仓库，使用与单仓库相同的工作台展示父目录文件树、各仓库分支与合并历史；选择后一键拉取，或统一预览、提交并推送，逐仓库报告结果。
...
| 多仓库工作区 | 递归发现子仓库与 `.git` 文件形式的工作树、跨仓库历史、批量 Fetch / Pull / Push、会话内授权、逐仓库进度和结果 |
...
**Push Preview** 左侧的每条待推送提交默认展开变更文件，显示路径及新增、修改、删除、重命名状态。点击文件弹出差异窗口，按该提交与第一父提交比较；支持统一 / 并排视图，重命名会包含旧路径，二进制文件显示专用提示。关闭弹窗或按 `Esc` 返回推送预览，单仓库右侧保留刚才查看的差异。

暂存区文件读取索引版本；多仓库 **Push selected** 同样支持查看各提交的文件，待提交文件展示 HEAD 到工作区的合并改动。同名文件按仓库及提交独立读取，查看差异不改变推送范围，也不清空已填写的提交说明。文件列表和差异读取失败时可重试。

### 0.9.1 多仓库统一工作台

打开一个没有 `.git` 的父文件夹，GitVista 会自动扫描子目录，并进入 **Multi-repository workspace**，不会给父目录自动执行 `git init`。父目录即使已有一个没有提交的空 Git 仓库，只要发现子仓库，也会改用工作区视图，避免向父目录的示例远端误拉取。已有正常仓库中也可通过仓库菜单的 **Scan subfolders…** 主动扫描。

![多仓库工作区：选择仓库、查看分支状态与跨仓库提交历史](docs/images/workspace-workbench-091.png)

*实际新版界面，图中的仓库、作者、历史和待提交文件采用演示数据。*

1. **打开父目录**：左上角选择 **Open local folder…**。进入与单仓库一致的工作台：左侧 **Repositories / Branches** 按项目展示所有发现的仓库、本地分支、远端分支和远端地址，可勾选批量操作范围，也可单独打开或设置授权。
2. **一起浏览文件和历史**：上方 **File Directory → All Files** 展示父文件夹完整目录，包括仓库外普通文件；展开目录按需加载，并隐藏 Git 内部元数据。底部复用单仓库的 **Commit History**，合并各仓库历史并保留独立分支图和仓库标签，支持分页、仓库 / 分支 / 作者 / 日期 / 路径筛选及可拖动列宽。相同哈希在不同仓库中分别显示。点击提交直接在当前页面展示详情和文件差异，**Changed Files** 可只看该提交的变更文件；点击完整目录中的文件则读取当前磁盘代码。
3. **一键拉取**：默认选择全部发现的仓库，也可取消部分勾选，然后点击 **Pull selected**。拉取前自动重新扫描，当前全选时也会纳入新加入的子仓库；取消全部勾选时不会执行拉取。按当前拉取策略逐仓库执行，默认只允许快进；不会自动丢弃、储藏改动或强制覆盖历史。
4. **一键推送更改**：点击 **Push selected**，检查每个仓库的目标分支、待推送提交和文件。默认包含未提交更改，填写统一提交说明后，依次提交列出的全部文件并推送；关闭该选项则只推送已有提交。无变化的仓库不执行写入，无法预览的仓库单独报错。
5. **看结果和重试**：底部显示逐仓库进度、成功 / 失败信息和输出。一个仓库失败后继续其他仓库；已完成的操作不会回滚。提交成功但推送失败会显示本地提交哈希，重新预览后可推送该提交，不会重复提交。

![批量推送预览：各仓库的目标、提交与文件列表](docs/images/workspace-push-090.png)

批量提交前会重新核对 HEAD、分支、推送地址、索引及工作区内容；预览后发生变化时要求重新预览。预览在十分钟后失效且只能执行一次。不进行强制推送或自动推送标签。存在冲突或未完成的合并 / 变基时，请先单独处理该仓库；空仓库需先完成首次提交后再批量推送。

**扫描范围**：识别 `.git` 目录及工作树的 `.git` 文件，不跟随目录联接或符号链接，不进入 `.git`、`node_modules`、`target`、`dist` 等依赖 / 构建目录。发现子仓库后停止向其内部递归，子模块由所属仓库管理。单次最多发现 100 个仓库、遍历 20,000 个目录、深入 12 层，约 20 秒达到限制时显示提示；打开更具体的子目录可继续。点击 **Rescan** 或 **Pull selected** 重新发现新增或移走的仓库。文件树的浏览不受这些依赖 / 构建目录排除规则影响；每个目录按需加载最多 10,000 项，文本预览限制为 4 MB，不跟随符号链接或目录联接。

工作区同样保存在最近列表，下次启动可恢复。托盘与迷你条会给汇总的最新提交标注仓库；从托盘拉取工作区时处理该目录下全部发现的仓库，主窗口按钮则按当前勾选执行。
...
| 入口 | 操作 |
| --- | --- |
| **Open local folder…** | 选择本地文件夹；普通父目录自动扫描子仓库，未发现仓库时可重新扫描或手动初始化 |
| **Clone from Git…** | 填写远端地址、父文件夹和可选目录名，点击 **Clone and open** 后自动打开结果 |
| **Scan subfolders…** | 将当前目录作为工作区，发现子目录中的仓库 |
...
点击 **Push** 先显示待推送提交及每条提交下的变更文件，点击文件弹出该次提交的差异；暂存区文件可独立查看已暂存差异。**如果暂存区有文件，直接推送会先按填写的提交说明提交整个暂存区，再推送**，请在预览中核对范围。推送失败不撤销已经完成的本地提交。
...
| `electron/workspace-service.ts`、`src/components/WorkspacePanel.tsx`、`src/components/WorkspaceWorkbench.tsx` | 子仓库发现、父目录浏览、统一工作台、批量预览与交互 |
...
多仓库功能另外使用隔离的本地仓库和本地远端验证：递归发现、工作树 `.git` 文件、目录联接排除、批量操作失败隔离、预览变更拒绝、提交后推送失败及重试。0.9.1 进一步验证父目录普通文件预览、跨仓库同哈希隔离、1200 条日志分页、路径筛选及统一工作台导航。验证不向实际项目远端推送。
